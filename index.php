          ...
          <table class="table table-hover">
            ...
            <tbody>
              <?php
               include 'model.php';
               $model = new Model();
               $rows = $model->fetch(); //la methode de recuperation de l,instance Mode en s'attendeant a ce qu'elle renvoie un tableau d'enrg de la bd
               $i = 1; //il semble que cette varable puisse etre utilisee coome le compteur dans la boucle
               if(!empty($rows)){ // checks if the variable os emply 
                 foreach($rows as $row){ 
             ?>
             <tr>
               <td><?php echo $i++; ?></td>
